...
exports.findOne = async (req,res)=>{
    ...
    try{
        ...
        if(data){
            ...
            res.status(409).send({ message: `No se encontro el registro con el id=${id}` });
        }
    }catch(err){
...
exports.update = async (req,res)=>{
    const id = req.params.id;
    try{
        const num = await EntSal.update(req.body,{ where:{idTransac:id} });
        if(num==1){
...
exports.delete = async (req,res)=>{
    const id = req.params.id
    try {
        const num =   await  EntSal.destroy({ where:{idTransac:id} });
        if(num==1){
